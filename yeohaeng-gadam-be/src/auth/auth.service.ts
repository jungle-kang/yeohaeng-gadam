...
export class authService {
    ...
    //     }
    // }
    async findByEmailOrSave(email: string, firstName: string, lastName: string, picture: string): Promise<User> {
        try {
    ...
    async googleLogin(req): Promise<any> {
        ...
        const user: User = await this.findByEmailOrSave(email, firstName, lastName, picture); // 이메일로 가입된 회원을 찾고, 없다면 회원가입
