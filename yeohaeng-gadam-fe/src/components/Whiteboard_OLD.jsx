  ...
  const calculateLineTime = async (line, transportMethod, shape1, shape2) => {
    ...
    switch (transportMethod) {
      ...
      case TRANS_METHOD_BUS:
        ...
        console.log("Google API req: ", `/maps/api/directions/json?destination=${shape2.get("placeY")},${shape2.get("placeX")}&origin=${shape1.get("placeY")},${shape1.get("placeX")}&departure_time=1714532400&mode=transit&key=${API_KEY}`);
        const res = await fetch(
          `/maps/api/directions/json?destination=${shape2.get("placeY")},${shape2.get("placeX")}&origin=${shape1.get("placeY")},${shape1.get("placeX")}&departure_time=1714532400&mode=transit&key=${API_KEY}`
        );
        const result = await res.json();
